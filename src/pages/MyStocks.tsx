...
function MyStocks() {
    ...
    const [stockAbbrev, setStockAbbrev] = useState("");
    ...
    return (
        <>
            <div className="flex flex-col mt-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-6">
                    {Object.entries(userData).map(([key, value]) => {
                        ...
                        return (
                            <div
                                className="bg-border-primary rounded-xl p-4 h-52 flex flex-col hover:scale-98 hover:cursor-pointer hover:shadow-md shadow-border-highlight transition-transform duration-200 "
                                key={key}
                                onClick={() => {
                                    setIsModal(true);
                                    setStockAbbrev(String(key));
                                }}
                            >
            ...
            {isModal && (
                ...
                >
                    <StockInfo stockAbbrev={stockAbbrev} />
                </Modal>
            )}
